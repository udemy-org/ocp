 ...
 * 
 *         A la hora de trabajar con tipos primitivos, int, double, etc., hay streams especializados que tratan con
 *         estos tipos primitivos.
 * 
 *         Las tablas 4.6, 4.7 y 4.8 del libro de referencia deben saberse de memória.
 * 
...
public class Leccion_04_06 {
	...
	/*
	 * También se puede crear un stream de primitivos desde un Stream con los métodos flatMapToInt(),
	 * flatMapToDouble() o flatMapToLong().
	 */
	List<Integer> list = Arrays.asList(1, 2, 3, 4, 5);
	IntStream ints = list.stream().flatMapToInt(x -> IntStream.of(x));

	/*
	 * Así como la clase Optional es genérica para utilizar con cualquier tipo envolvente, también disponemos de
	 * clases Optional para tipos primitivos: OptionalDouble, OptionalInt i OptionalLong. Todas ellas cuentan con
	 * métodos similares a la clase Optional.
	 */
	OptionalDouble odouble = IntStream.rangeClosed(1, 10).average();
	odouble.ifPresent(out::println);

	/*
	 * Tanto max() como min() son opereciones terminales que por decirlo de alguna manera, consumen el stream. Por
	 * lo tanto no podríamoso calcular la diferencia entre el número mayor y el menor de un stream.
	 * 
	...
	/*
	 * Así como hay Optionals para algunos primitivos, también hay interfaces funcionales para primitivos:
	 * suppliers, consumers, predicates, functions, unary operators, binary operators. Todo menos biconsumers,
	 * bipredicates y bifunctions.
	 * 
	 */
	...
	/*
	 * Y la lista sigue y sigue. Consulta las tablas 4.9 y 4.10 del manual de referencia
	 */
